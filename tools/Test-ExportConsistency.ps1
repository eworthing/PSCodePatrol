...
$ruleExports = @(
    Get-ChildItem -LiteralPath $RulesPath -Filter '*.ps1' -File |
    ...
    if ($missingInManifest.Count -gt 0 -or $missingRuleFile.Count -gt 0) {
        if ($missingInManifest.Count -gt 0) {
            Write-Output 'Rules missing in FunctionsToExport:'
            $missingInManifest | ForEach-Object { Write-Output "  - $_" }
        }

        if ($missingRuleFile.Count -gt 0) {
            Write-Output 'FunctionsToExport entries missing corresponding rule files:'
            $missingRuleFile | ForEach-Object { Write-Output "  - $_" }
        }

        throw 'FunctionsToExport is out of sync with Rules/*.ps1.'
    }

    Write-Output 'FunctionsToExport matches Rules/*.ps1.'
